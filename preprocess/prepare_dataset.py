...
data_dict, label_names = load_texts_with_labels(INPUT_DIR)
dataset = Dataset.from_dict(data_dict)
print(Counter(dataset["label"]))
print(label_names)
split_dataset_dict = dataset.class_encode_column("label").train_test_split(
    test_size=0.15, shuffle=True, stratify_by_column="label"
)


def decode_labels(example):
    example["label_string"] = (
        split_dataset_dict["train"].features["label"].int2str(example["label"])
    )
    return example
...
train_dataset = (
    train_dataset.remove_columns("label")
    .rename_column("label_string", "answer")
    .rename_column("text", "question")
)
test_dataset = (
...
test_dataset.to_csv(f"{OUTPUT_DIR}/test.csv", index=False)

print(f"Files saved to {OUTPUT_DIR}/train.csv and {OUTPUT_DIR}/test.csv")
